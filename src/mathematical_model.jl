...
function bid_based_market_clearing_model_action(args...)
    ...
    if any_valid_elements(inputs, run_time_options, HydroUnit, action; filters = [!has_bidding_group])
        hydro_generation!(args...)
        hydro_volume!(args...)
        hydro_inflow!(args...)
        if any_valid_elements(
            ...
            HydroUnit,
            action;
            filters = [!has_bidding_group, has_commitment],
        )
            hydro_commitment!(args...)
        end
    end
    if any_valid_elements(inputs, run_time_options, ThermalUnit, action; filters = [!has_bidding_group])
        thermal_generation!(args...)
        if any_valid_elements(
            inputs,
            run_time_options,
    ...
    if any_valid_elements(inputs, run_time_options, RenewableUnit, action; filters = [!has_bidding_group])
        renewable_generation!(args...)
    end
    if any_valid_elements(inputs, run_time_options, BatteryUnit, action; filters = [!has_bidding_group])
        battery_unit_generation!(args...)
        battery_unit_storage!(args...)
    end
