...
# In this tutorial we will start with the [Case 1](build_base_case.md) elements and build a run of river Hydro Plant.
# This Hydro Plant will be linked to a new Bidding Group `Hydro Owner Group`, which belongs to the `Hydro Owner` owner.
...
# As already mentioned, we will add a Hydro Plant to the case. This Hydro Plant will belong to a new a new Asset Owner called `Hydro Owner`.

# Moreover, we will change the number of scenarios to 12, considering that the inflow values for Hydro Plant could be on High, Medium or Low levels.
...
IARA.add_bidding_group!(
    ...
    simple_bid_max_segments = 1, # number of plants
)
...
# For this example, we will add a Hydro Plant with the following characteristics
...
#md # !!! note "Note"  
#md #     A Run of river Hydro Plant does not have a reservoir

# To add a hydro plant, we need to use the [`IARA.add_hydro_plant!`](@ref) function.
...
# Just as in the [previous tutorial](build_base_case.md), we will be loading a CSV file we have created earlier.
