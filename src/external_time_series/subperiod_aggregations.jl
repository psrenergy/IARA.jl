...
function apply_hour_subperiod_map(
    ...
    return aggregate_data(inputs, data, new_data, variable_aggregation_type(unit), hour_dimension_index)
end
...
function aggregate_data(
    ...
    old_data::Array{T, 2},
    new_data::Array{T, 2},
    variable_aggregation_type::Configurations_VariableAggregationType.T,
    hour_dimension_index::Int,
) where {T}
    ...
    for subperiod in subperiods(inputs)
        ...
        if variable_aggregation_type == Configurations_VariableAggregationType.SUM
            new_data[:, subperiod] = sum(old_data[:, hour] for hour in hours_in_subperiod)
        elseif variable_aggregation_type == Configurations_VariableAggregationType.AVERAGE
            new_data[:, subperiod] = mean(old_data[:, hour] for hour in hours_in_subperiod)
        elseif variable_aggregation_type == Configurations_VariableAggregationType.LAST_VALUE
