...
function lp_filename(
    ...
    if !isdir(debug_path(inputs))
        mkpath(debug_path(inputs))
    end
    ext = isempty(suffix) ? ".lp" : "_$suffix.lp"
    if is_reference_curve(inputs, run_time_options)
        return joinpath(debug_path(inputs), "t$(t)_s$(scen)_reference_curve$(ext)")
    elseif run_mode(inputs) == RunMode.TRAIN_MIN_COST
        return joinpath(debug_path(inputs), "t$(t)_s$(scen)_train_min_cost$(ext)")
