...
function fill_initial_energy_account!(inputs::AbstractInputs, vr::Int)
    ...
    total_energy_account = sum(
        (hydro_unit_initial_volume(inputs, h) - hydro_unit_min_volume(inputs, h)) *
        virtual_reservoir_water_to_energy_factors(inputs, vr, h) for
        h in hydro_units
    )
...
function calculate_energy_account_and_spilled_energy!(
    ...
    for vr in virtual_reservoirs
        ...
        total_actual_energy_account = sum(
            ((hydro_volume[end, h] - hydro_unit_min_volume(inputs, h))) *
            virtual_reservoir_water_to_energy_factors(inputs, vr, h) for
