...
function pricemaker_revenue(
    ...
    merit_order = sortperm(price_offers)
    sums = cumsum(quantity_offers[merit_order])
    duals = price_offers[reverse!(merit_order)]
    if has_contracts
    ...
    for e in Iterators.reverse(sums)
        ...
        if offer_energy > 0
            ...
            if has_contracts
                ...
                revenue = fixed_contract_revenue - total_contracted_quantity * duals[i] + offer_energy * duals[i]
                push!(points, Point(offer_energy, revenue))
                if i < N
                    revenue =
                        fixed_contract_revenue - total_contracted_quantity * duals[i+1] + offer_energy * duals[i+1]
                    push!(points, Point(offer_energy, revenue))
                end
            else
                if first
