...
# In the [previous section](build_hydrounit_base_case.md), we have loaded the base case and added a Hydro Unit to it. Now, we will run this case in two different run modes:
...
#md # !!! note "Note"
#md #     As we have a Hydro Unit in this case, running the `CENTRALIZED_OPERATION` mode before the `MARKET_CLEARING` mode is necessary, as it will help us generate the hydro offers.
