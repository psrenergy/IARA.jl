...
The maximum energy generation in the period and scenario is determined by the amount of stored energy, the turbine limits of the reservoirs and the total demand of the period. A set of multipliers $\Theta$ is defined, ranging uniformly from 0 to 1, and the amount of elements is defined by the user. Each multiplier $\theta \in \Theta$ is applied to the maximum amount of available energy. For each value of the multiplier $\theta$, a single optimization model $Z(\theta)$ is run, which returns the quantity $q_r^*$ for each virtual reservoir and the price $\mu$ in the scenario considered.
...
- ``\mu_r(\theta) = \mu^*``, which represents the price for the multiplier $\theta$.
...
## Parameters
- ``\hat{q}_r(\omega)``: Available energy of virtual reservoir $r$ for current period, scenario $\omega$. Calculated as the minimum between the total stored energy at $r$ and the maximum turbinable energy at $r$ considering the period duration: $\hat{q}_r(\omega) = \min(\sum_{i \in I^{VR}(r)} E_{r,i}\; , \; \sum_{h \in J^{H}_{VR}(r)} \sum_{\tau \in B(t)} \rho_h \cdot U_h \cdot d(\tau))$
- ``D(\omega)``: Total demand energy of the period: $D(\omega) = \sum_{j \in J^{DI} \cup J^{DE} \cup J^{DF}} \sum_{\tau \in B(t)} D_{j,\tau}(\omega)$
- ``\bar{Q}(\omega)``: Total reference energy of the period, which is the width of the reference curve: $\bar{Q}(\omega) = \min\left(\sum_{r \in J^{VR}} \hat{q}_r(\omega)\; , \; D(\omega)\right)$. The demand cap keeps the multipliers $\Theta$ sampling the energy that the market could absorb, and is not applied when there is no demand in the period.
...
```math
    \sum_{\tau \in B(t)}{\sum_{h \in J^H_{VR}(r)} \rho_h u_{h, \tau} } = q_r \quad \forall r \in J^{VR}
```

### Generation reference

```math
    \sum_{r \in J^{VR}} q_r = \theta \cdot \bar{Q}(\omega) \quad :\mu
```
...
For each asset owner $i$ in the virtual reservoir $r$, the heuristic bid is constructed based on the hydro reference curve $[Q_{r, g}, P_{r,g}]_{g \in G}$, the pairs of reference and markup $(s_{i,f}, m_{i,f})_{f \in F^{AO}(i)}$, the purchase discount rate $o_i$, and the energy accounts $E^{in}_r$.
