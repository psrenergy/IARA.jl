...
function reinitialize_bids_time_series_for_nash_iteration!(
    ...
    if !isfile(file * ".qvr")
        ...
        source_base = joinpath(path_case(inputs), bidding_group_quantity_bid_file(inputs))
        convert_time_series_file_to_binary(source_base)
        source_base = resolve_binary_file_path(source_base)
        cp(source_base * ".qvr", file * ".qvr")
        cp(source_base * ".toml", file * ".toml")
    end
    num_errors += initialize_bids_view_from_external_file!(
        inputs.time_series.quantity_bid,
        inputs,
    ...
    if !isfile(file * ".qvr")
        source_base = joinpath(path_case(inputs), bidding_group_price_bid_file(inputs))
        convert_time_series_file_to_binary(source_base)
        source_base = resolve_binary_file_path(source_base)
        cp(source_base * ".qvr", file * ".qvr")
        cp(source_base * ".toml", file * ".toml")
...
function update_segments_profile_dimensions_by_timeseries!(inputs, period)
    ...
    for scenario in 1:number_of_scenarios(inputs)
        ...
        # TODO: add enum value to read VR bids from file
