...
# In the [previous section](build_hydroplant_base_case.md), we have loaded the base case and added a Hydro Plant to it. Now, we will run this case in two different run modes:
...
#md # !!! note "Note"
#md #     As we have a Hydro Plant in this case, running the `CENTRALIZED_OPERATION` mode before the `MARKET_CLEARING` mode is necessary, as it will help us generate the hydro offers.
...
# Now that we have the hydro offers from the `CENTRALIZED_OPERATION` mode, we can run the `MARKET_CLEARING` mode.

# Let's copy the data from the `CENTRALIZED_OPERATION` mode to a new folder. 
