...
# 
# $GAME_PERIOD: The period to be executed. It should be an integer
#
# Usage:
#  $ ./IARA_ecs.sh
...
function download_and_unzip_case () {
    ...
    aws s3 cp s3://$S3_BUCKET/games/$IARA_CASE/game_inputs.zip ./$IARA_CASE.zip > /dev/null 2>&1
...
if [ "$IARA_COMMAND" == "heuristic bid" ]; then
    ...
    $IARA_PATH/IARA.sh $CASE_PATH --output-path $CASE_PATH/heuristic_bids --run-mode 'single-period-heuristic-bid' --period $GAME_PERIOD # TODO missing other arguments
...
if [ "$IARA_COMMAND" == "single period market clearing" ]; then
    ...
    $IARA_PATH/IARA.sh $CASE_PATH --run-mode 'single-period-market-clearing' --period $GAME_PERIOD # TODO missing other arguments
