...
- ``K^{VR}(r, i)``: Set of segment bids at virtual reservoir $r$ for the asset owner $i$.
...
- ``P_{i, n, \tau, k}(\omega)``: Price bid of asset owner $i$ on network node $n$ during subperiod $\tau$, segment $k$ and scenario $\omega$.
- ``Q_{i, n, \tau, k}(\omega)``: Quantity bid of asset owner $i$ on network node $n$ during subperiod $\tau$, segment $k$ and scenario $\omega$.

### Profile Bids

- ``P^M_{i, n, k}(\omega)``: Price bid of asset owner $i$ on network node $n$, profile $k$ and scenario $\omega$.
- ``Q^M_{i, n, \tau, k}(\omega)``: Quantity bid of asset owner $i$ on network node $n$ during subperiod $\tau$, profile $k$ and scenario $\omega$.
...
### Virtual Reservoirs
- ``P^{VR}_{r, i, k}(\omega)``: Price bid of asset owner $i$ on virtual reservoir $r$ for segment $k$ at scenario $\omega$.
- ``Q^{VR}_{r, i, k}(\omega)``: Quantity bid of asset owner $i$ on virtual reservoir $r$ for segment $k$ at scenario $\omega$.
...
- ``\lambda_{i, n, \tau, k}``: Linear combination coefficients for segment bid $k$ of asset owner $i$ on network node $n$ during subperiod $\tau$.
- ``q_{i, n, \tau, k}``: Total energy generated by asset owner $i$ on network node $n$ during subperiod $\tau$ related to the segment bid $k$.
...
- ``q^{VR}_{r, i, k}``: Total energy generated by asset owner $i$ on virtual reservoir $r$ related to segment bid $k$.
